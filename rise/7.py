...
# Registered devices for validation (disabled mac validation for public release)
...
# Key constants for simulation
R = 0x52
Z = 0x5A
ONE = 0x31
current_camp = 1
loading_screen_duration = 10 + 5  # 5 seconds for character flashing
next_camp_change_timedelta = 10
...
def get_camp_header():
    return 1802, 53  # Fixed x, y for 1920x1080


def get_camp_header2():
    return 1802, 74  # Fixed x, y for 1920x1080
...
def click_camp_header2():
    x, y = get_camp_header2()
    mouse_click(target_x=x, target_y=y)
...
def click_next_camp2():
    click_x, click_y = get_camp_header2()
    _current_camp = 1 if current_camp == max_camp else current_camp
    next_camp_y = click_y + (20 * _current_camp)
    mouse_click(target_x=click_x, target_y=next_camp_y, times=3)
...
if __name__ == '__main__':
    print('Set the game to windowed 1920x1080')
    print('If you haven’t set the resolution yet, please close and reopen the program.')
    print('To pause/resume, press the RIGHT CTRL and RIGHT ALT keys simultaneously.')
